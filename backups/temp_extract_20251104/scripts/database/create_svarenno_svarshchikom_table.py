    ...
    def populate_table(self):
        ...
        try:
            ...
            if self.has_bazovyy_material:
                ...
                material_check = "(COALESCE(wc.Базовый_материал_1, '') NOT LIKE '%09Г2C%' AND COALESCE(wc.Базовый_материал_1, '') NOT LIKE '%09Г2С%' AND COALESCE(wc.Результаты_Заключения_РК, '') != 'Не годен')"
                ...
                material_check = "(COALESCE(wc.Результаты_Заключения_РК, '') != 'Не годен')"
            ...
            SELECT
                ...
                {f"SUM(CASE WHEN ((COALESCE(wc.Базовый_материал_1, '') LIKE '%09Г2C%' OR COALESCE(wc.Базовый_материал_1, '') LIKE '%09Г2С%') OR COALESCE(wc.Результаты_Заключения_РК, '') = 'Не годен') THEN 1 ELSE 0 END)" if self.has_bazovyy_material else f"SUM(CASE WHEN COALESCE(wc.Результаты_Заключения_РК, '') = 'Не годен' THEN 1 ELSE 0 END)"} as исключено_по_материалу_стило,

                SUM(CASE WHEN wc.Результаты_АКТ_ВИК = 'Заказ отправлен' THEN 1 ELSE 0 END) as заявлен_вик,

                SUM(CASE WHEN wc.Результаты_АКТ_ВИК = 'годен' THEN 1 ELSE 0 END) as годен_вик,

                SUM(CASE WHEN wc.Результаты_АКТ_ВИК = 'Не годен' THEN 1 ELSE 0 END) as не_годен_вик,

                SUM(CASE WHEN wc.Результаты_АКТ_ВИК IS NULL OR wc.Результаты_АКТ_ВИК = '' OR wc.Результаты_АКТ_ВИК = 'None' THEN 1 ELSE 0 END) as не_подан_вик,

                SUM(CASE WHEN wc.Результаты_Заключения_РК = 'Заказ отправлен' THEN 1 ELSE 0 END) as заявлен_рк,

                SUM(CASE WHEN wc.Результаты_Заключения_РК = 'годен' THEN 1 ELSE 0 END) as годен_рк,

                SUM(CASE WHEN wc.Результаты_Заключения_РК = 'Не годен' THEN 1 ELSE 0 END) as не_годен_рк,
                ...
                SUM(CASE WHEN wc.Результаты_Заключения_Стилоскопирование = 'Заказ отправлен' AND {material_check} THEN 1 ELSE 0 END) as заявлен_стилоскопирование,

                SUM(CASE WHEN wc.Результаты_Заключения_Стилоскопирование = 'годен' AND {material_check} THEN 1 ELSE 0 END) as годен_стилоскопирование,

                SUM(CASE WHEN wc.Результаты_Заключения_Стилоскопирование = 'Не годен' AND {material_check} THEN 1 ELSE 0 END) as не_годен_стилоскопирование,

                SUM(CASE WHEN (wc.Результаты_Заключения_Стилоскопирование IS NULL OR wc.Результаты_Заключения_Стилоскопирование = '' OR wc.Результаты_Заключения_Стилоскопирование = 'None') AND {material_check} THEN 1 ELSE 0 END) as не_подан_стилоскопирование,

                SUM(CASE WHEN wc.Результаты_Заключения_МПД = 'Заказ отправлен' THEN 1 ELSE 0 END) as заявлен_мпд,

                SUM(CASE WHEN wc.Результаты_Заключения_МПД = 'годен' THEN 1 ELSE 0 END) as годен_мпд,

                SUM(CASE WHEN wc.Результаты_Заключения_МПД = 'Не годен' THEN 1 ELSE 0 END) as не_годен_мпд,

                SUM(CASE WHEN wc.Результаты_Заключения_МПД IS NULL OR wc.Результаты_Заключения_МПД = '' OR wc.Результаты_Заключения_МПД = 'None' THEN 1 ELSE 0 END) as не_подан_мпд

...
def get_db_connection():
    ...
    print(f"[DB] Попытка подключения к БД: {app.config['DB_PATH']}")
    try:
        ...
        print("[DB] Подключение к БД успешно")
        return conn
    except sqlite3.OperationalError as e:
        print(f"[ERROR] Ошибка подключения к БД (OperationalError): {e}")
        logger.error(f"Ошибка подключения к БД (OperationalError): {e}")
        if "database is locked" in str(e).lower():
            print("[ERROR] База данных заблокирована другим процессом")
...
def view_table(table_name):
    ...
    try:
        ...
        print(f"[DEBUG] Передаем в шаблон: all_columns={len(all_columns) if 'all_columns' in locals() else 'НЕ ОПРЕДЕЛЕНА'}, selected_columns={len(columns_to_show) if 'columns_to_show' in locals() else 'НЕ ОПРЕДЕЛЕНА'}")
        if 'all_columns' in locals():
            print(f"[DEBUG] all_columns: {all_columns}")
        else:
            print(f"[DEBUG] all_columns НЕ ОПРЕДЕЛЕНА")
        if 'columns_to_show' in locals():
            print(f"[DEBUG] columns_to_show: {columns_to_show}")
        else:
            print(f"[DEBUG] columns_to_show НЕ ОПРЕДЕЛЕНА")
        ...
        print(f"[ERROR] Ошибка в view_table: {e}")
        import traceback
...
def process_wl_china_numbers():
    """API для обработки таблицы wl_china и извлечения чисел из столбца Номер_сварного_шва"""
    try:
        conn = get_db_connection()
        if not conn:
            return jsonify({'error': 'Ошибка подключения к БД'})
        ...
        # Добавляем столбец _Номер_сварного_шва, если его нет
        try:
            cursor.execute('ALTER TABLE wl_china ADD COLUMN _Номер_сварного_шва TEXT')
            print("✅ Добавлен новый столбец '_Номер_сварного_шва' в wl_china")
        except Exception as e:
            if "duplicate column name" in str(e).lower():
                print("ℹ️ Столбец '_Номер_сварного_шва' уже существует в wl_china")
            else:
...
def api_weld_repair_log_import():
    ...
    try:
        ...
        has_app_row_id = any(c.lower() in ['app_row_id', 'id'] for c in df.columns)
        if not has_app_row_id:
        ...
        for _, r in df.iterrows():
            ...
            # Найдем поле ID
            id_col = next((c for c in df.columns if c.lower() in ['app_row_id', 'id']), None)
            ...
            if pd.isna(record_id) or record_id == '':
                ...
                cursor.execute('SELECT COUNT(*) FROM weld_repair_log WHERE app_row_id = ?', (int(record_id),))
                if cursor.fetchone()[0] == 0:
                    continue
                ...
                if update_parts:
                    update_values.append(int(record_id))
        ...
        return jsonify({
            'success': True,
            'message': f'Импорт завершен. Обработано: {processed}, обновлено: {updated}, создано: {created}',
            'processed': processed,
            'updated': updated,
            'created': created
        })
...
def api_import_nk_results():
    ...
    try:
        ...
        id_column = None
        for candidate in ['ID', 'id', 'app_row_id', 'AppRowId', 'App_Row_Id']:
        ...
        for _, row in df.iterrows():
            ...
            cursor.execute('SELECT COUNT(*) FROM weld_repair_log WHERE app_row_id = ?', (record_id_int,))
            if cursor.fetchone()[0] == 0:
                continue

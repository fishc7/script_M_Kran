...
# -*- coding: utf-8 -*-
"""
Скрипт для нормализации столбцов Статус_РК и РК в таблице logs_lnk

Задачи:
1. Если Статус_РК пустой, то брать не пустое значение из столбца РК
2. Привести значения Статус_РК к общему виду согласно маппингу
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/normalize_rk_status_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log', encoding='utf-8'),
...
def normalize_rk_status():
    ...
    """
    logger.info("🚀 Начинаем нормализацию столбцов Статус_РК и РК")
    ...
    try:
        ...
        if 'Статус_РК' not in column_names:
            logger.error("❌ Столбец 'Статус_РК' не найден в таблице logs_lnk")
            return False
        
        if 'РК' not in column_names:
            logger.error("❌ Столбец 'РК' не найден в таблице logs_lnk")
            return False
        
        logger.info("✅ Столбцы 'Статус_РК' и 'РК' найдены в таблице")
        ...
        logger.info("🔄 Шаг 1: Заполняем пустые Статус_РК значениями из столбца РК")
        ...
        logger.info(f"📊 Найдено записей с пустым Статус_РК и непустым РК: {empty_status_count}")
        ...
        logger.info("🔄 Шаг 2: Нормализуем значения Статус_РК согласно маппингу")
        ...
        logger.info(f"📊 Уникальные значения Статус_РК: {unique_statuses}")
        ...
        logger.info("📊 Финальная статистика по Статус_РК:")
...
def main():
    """Основная функция"""
    logger.info("=" * 60)
    logger.info("НОРМАЛИЗАЦИЯ СТОЛБЦОВ СТАТУС_РК И РК")
    logger.info("=" * 60)
    
    success = normalize_rk_status()

...
SAVE_PATH = r"d:\МК_Кран\script_M_Kran\NAKS\НАКС_парсинг"
    ...
    if results:
        ...
        try:
            ...
            def safe_subprocess_run(cmd, **kwargs):
                """Безопасный запуск subprocess с автоматическим определением кодировки"""
                try:
                    return subprocess.run(cmd, encoding='utf-8', **kwargs)
                except UnicodeDecodeError:
                    return subprocess.run(cmd, encoding='cp1251', **kwargs)
            
            result = safe_subprocess_run([sys.executable, join_script_path], 
                                      capture_output=True, 
                                      text=True, 

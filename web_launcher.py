...
def cleanup_processes():
    ...
    if html_server:
        try:
            ...
            if hasattr(html_server, 'stop'):
                html_server.stop()
            elif hasattr(html_server, 'terminate'):
                html_server.terminate()
                if hasattr(html_server, 'wait'):
...
def open_browser_safe(url):
    ...
    # На Windows самый надежный способ открыть URL — os.startfile.
    if sys.platform.startswith('win'):
        print("   Пробуем открытие через os.startfile...")
        try:
            os.startfile(url)
            print("[OK] Браузер открыт через os.startfile")
            browser_opened = True
            time.sleep(2)
            return
        except Exception as e:
            print(f"   os.startfile не сработал: {e}")
    
    # Fallback через cmd/start (важно передавать команду строкой, а не list)
    print("   Пробуем принудительное открытие через cmd /c start...")
    try:
        ...
        if result.returncode == 0:
            print("[OK] Браузер открыт через cmd/start")
            browser_opened = True
            time.sleep(2)
            return
        print(f"   cmd/start вернул код: {result.returncode}")
    except Exception as e:
        print(f"   cmd/start не сработал: {e}")
...
def main():
    ...
    try:
        if is_port_busy(5000):
            print("[WARNING] Порт 5000 занят. Останавливаем старые процессы...")
            try:
                subprocess.run([sys.executable, 'stop_servers.py'], timeout=10, check=False)
                ...
                print(f"Попробуйте вручную: {sys.executable} stop_servers.py")
    except Exception as e:
    ...
    if is_port_busy(5000):
        selected_port = 5051
        print("[WARNING] Порт 5000 занят другим процессом, используем порт 5051")
    ...
    try:
        ...
        clean_env['MKRAN_PORT'] = str(selected_port)
        ...
        # Стабильный режим: без авто-открытия браузера.
        print(f"\n[INFO] Откройте главную страницу вручную: {url}")

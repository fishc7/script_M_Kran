...
class _LiveCaptureStream:
    """Прокси-поток: пишет в буфер и в live-callback для веб-лога."""

    def __init__(self, buffer: StringIO, on_write):
        self._buffer = buffer
...
class WebScriptRunner:
    ...
    def __init__(self, project_root: str):
        ...
        self.max_live_log_lines = 3000

    def _append_live_output(self, script_id: str, text: str, is_error: bool = False):
        """Добавляет фрагмент вывода в статус скрипта во время выполнения."""
        if not text:
            return
        with self._scripts_lock:
            ...
            if not status:
                return
            key = 'errors' if is_error else 'output'
            status[key] = (status.get(key, '') or '') + text
            live_log = status.setdefault('live_log', [])
            prefix = '[stderr] ' if is_error else ''
            normalized = str(text).replace('\r\n', '\n').replace('\r', '\n')
            for line in normalized.split('\n'):
                if not line:
                    continue
                live_log.append(f'{prefix}{line}')
            if len(live_log) > self.max_live_log_lines:
                del live_log[:len(live_log) - self.max_live_log_lines]
    ...
    def run_script_direct_import(self, script_path: str, script_id: str, script_args: list = None) -> Dict[str, Any]:
        ...
        live_output_stream = _LiveCaptureStream(
            captured_output,
            lambda chunk: self._append_live_output(script_id, chunk, is_error=False)
        )
        live_error_stream = _LiveCaptureStream(
            captured_errors,
            lambda chunk: self._append_live_output(script_id, chunk, is_error=True)
        )
        ...
        try:
            ...
            sys.stdout = live_output_stream
            sys.stderr = live_error_stream
            ...
            # В Python 3.14 dataclass может обращаться к sys.modules[cls.__module__]
            # во время exec_module. Регистрируем модуль заранее, как при обычном import.
            module_name = spec.name or "script_module"
            ...
            if hasattr(module, 'run_script'):
                ...
                # Для внешних скриптов часто есть только main() под __name__ == '__main__'.
                # В режиме import это условие не срабатывает, поэтому вызываем main() явно.
                if hasattr(module, 'main') and callable(module.main):
                    self.logger.info(f"[{script_id}] run_script() не найден, вызываем main()...")
                    try:
                        sig = inspect.signature(module.main)
                        # Если main ожидает argv, передаем список аргументов без имени скрипта
                        if len(sig.parameters) >= 1:
                            module.main(script_args or [])
                        else:
                            module.main()
                    except Exception:
                        # Fallback: пробуем без аргументов
                        module.main()
                else:
                    self.logger.info(f"[{script_id}] Модуль выполнен успешно (без run_script/main)")
    ...
    def run_script_async(self, script_path: str, script_args: list = None) -> str:
        ...
        def run_script():
            try:
                ...
                # Сохраняем результат
                with self._scripts_lock:
                    current = self.running_scripts.get(script_id, {})
                    result['live_log'] = current.get('live_log', [])
                    result['start_time'] = current.get('start_time', time.time())

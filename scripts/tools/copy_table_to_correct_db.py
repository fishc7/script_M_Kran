...
def copy_table_to_correct_database():
    ...
    source_db = project_root / "database" / "M_Kran_Kingesepp.db"
    target_db = project_root / "database" / "BD_Kingisepp" / "M_Kran_Kingesepp.db"
    ...
    try:
        ...
        source_cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='слов_клейм_факт'"
        )
        ...
        for record in records:
            print(
                f"  ID: {record[0]}, Клеймо: {record[1]}, ФИО: {record[2]}, Примечание: {record[3]}"
            )

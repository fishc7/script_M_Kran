...
try:
    ...
    # Fallback на старый способ, если новый модуль недоступен
...
def get_duplicates_wl_china(conn, logger):
    """
    Извлекает дубликаты из таблицы wl_china по полям «Номер_чертежа» и ключу стыка
    (COALESCE извлечённого _Номер_сварного_шва_без_S_F_ и исходного Номер_сварного_шва).
    """
    query = """
    WITH base AS (
        SELECT
            *,
            COALESCE(
                NULLIF(TRIM("_Номер_сварного_шва_без_S_F_"), ''),
                NULLIF(TRIM("Номер_сварного_шва"), ''),
                ''
            ) AS _styk_dup_key
        FROM wl_china
    ),
    duplicates AS (
        SELECT
            *,
            COUNT(*) OVER (PARTITION BY Номер_чертежа, _styk_dup_key) AS cnt
        FROM base
    )
    SELECT
        блок_,
        ...
        _Линии,
        Номер_чертежа,
        _styk_dup_key AS _Номер_сварного_шва,
        cnt as duplicate_count,
        id as original_id_china
    FROM duplicates
    WHERE cnt > 1
    ORDER BY Номер_чертежа, _styk_dup_key
    """
    
    try:
        df = pd.read_sql_query(query, conn)
        logger.info(
            "Найдено %s записей дубликатов в wl_china по полям «Номер_чертежа» и ключу стыка "
            "(извлечённый номер или «Номер_сварного_шва»)",
            len(df),
        )
        return df
    except Exception as e:
...
def main():
    ...
    try:
        ...
        logger.info("Поиск дубликатов в wl_china по «Номер_чертежа» и ключу стыка (без отдельного столбца _Номер_сварного_шва)")

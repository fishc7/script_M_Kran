...
def find_limit_dates():
    ...
    def find_limit_reach_date(group_data):
        ...
        for idx, row in group_data.iterrows():
            ...
            if cumulative_sum >= limit and limit_reach_date is None:
                # Находим дату, где количество было близко к лимиту, но меньше
                if idx > 0:
                    # Берем предыдущую дату, где количество было меньше лимита
                    limit_reach_date = group_data.iloc[idx-1]['Дата']
                    limit_reach_count = cumulative_sum - row['Кол_во']
                else:
                    # Если это первая запись, берем текущую дату
                    limit_reach_date = row['Дата']
                    limit_reach_count = cumulative_sum
                
                # Проверяем, что количество на дату лимита действительно меньше лимита
                if limit_reach_count >= limit:
                    # Ищем дату, где количество было меньше лимита
                    for i in range(idx-1, -1, -1):
                        temp_sum = 0
                        for j in range(i+1):
                            temp_sum += group_data.iloc[j]['Кол_во']
                        if temp_sum < limit:
                            limit_reach_date = group_data.iloc[i]['Дата']
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        material_summary = df_limit_analysis.groupby('материал').agg({
            'Общее_количество': 'sum',
            'Превышение': 'sum',
            'клеймо': 'nunique'
        }).reset_index()
        material_summary.columns = ['Материал', 'Общее количество', 'Общее превышение', 'Количество сварщиков']
        material_summary.to_excel(writer, sheet_name='Сводка по материалам', index=False)
        
        # Сводка по сварщикам
        welder_summary = df_limit_analysis.groupby(['материал', 'клеймо']).agg({
            'Общее_количество': 'sum',
            'Превышение': 'sum',
            'Группа_диаметра': 'nunique'
        }).reset_index()
        welder_summary.columns = ['Материал', 'Клеймо сварщика', 'Общее количество', 'Общее превышение', 'Количество групп диаметров']
    ...
    # Показываем группы с превышением лимитов
    if len(exceeded_limits) > 0:
        print(f"\n8. Группы с превышением лимитов:")
        for idx, row in exceeded_limits.iterrows():
            print(f"   {row['материал']} | {row['клеймо']} | {row['способ_сварки']} | {row['Группа_диаметра']}")
            print(f"     Лимит: {row['Лимит']}, Достигнут: {row['Дата_достижения_лимита'].strftime('%d.%m.%Y')}")
            print(f"     На дату лимита: {row['Количество_на_дату_лимита']}, Общее: {row['Общее_количество']}, Превышение: {row['Превышение']}")
            print()

...
def calculate_multiple_batches():
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        # Сводка по группам
        group_summary = df_batches.groupby(['материал', 'клеймо', 'способ_сварки', 'Группа_диаметра']).agg({
            'Номер_партии': 'max',
            'Накопительное_количество': 'max',
            'Количество_в_партии': 'sum'
        }).reset_index()
        group_summary.columns = ['Материал', 'Клеймо сварщика', 'Способ сварки', 'Группа диаметра', 'Количество партий', 'Общее количество', 'Сумма по партиям']
        group_summary.to_excel(writer, sheet_name='Сводка по группам', index=False)
    ...
    print(f"   Общее количество стыков: {df_batches['Накопительное_количество'].max()}")
    
    # Показываем группы с несколькими партиями
    multi_batch_groups = group_summary[group_summary['Количество партий'] > 1]
    if len(multi_batch_groups) > 0:
        print(f"\n8. Группы с несколькими партиями:")
        for idx, row in multi_batch_groups.iterrows():
            print(f"   {row['Материал']} | {row['Клеймо сварщика']} | {row['Способ сварки']} | {row['Группа диаметра']}")
            print(f"     Партий: {row['Количество партий']}, Общее количество: {row['Общее количество']}")
            print()

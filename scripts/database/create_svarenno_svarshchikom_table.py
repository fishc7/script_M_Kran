...
project_root = os.path.dirname(os.path.dirname(current_dir))  # На два уровня выше: scripts/database -> scripts -> project_root
...
class SvarennoSvarshchikomCreator:
    ...
    def populate_table(self):
        ...
        try:
            ...
            # Все швы считаются вместе (исключения по типу У17/У19 и по материалу 09Г2С/РК не применяются)

            # SQL запрос для заполнения таблицы

            insert_sql = """

            INSERT INTO сварено_сварщиком (N_Линии, _Линия, Номер_чертежа, Всего_сваренно_сварщиком, "Не_Внесенно(не_сваренно)", клейма_сварщиков, исключено_по_типу, исключено_по_материалу_стило, заявлен_вик, годен_вик, не_годен_вик, не_подан_вик, заявлен_рк, годен_рк, не_годен_рк, не_подан_рк, заявлен_pt, годен_pt, не_годен_pt, не_подан_pt, заявлен_стилоскопирование, годен_стилоскопирование, не_годен_стилоскопирование, не_подан_стилоскопирование, заявлен_мпд, годен_мпд, не_годен_мпд, не_подан_мпд)
            ...
            SELECT
                ...
                COUNT(DISTINCT CASE 
                    WHEN TRIM(COALESCE(wc.Результаты_АКТ_ВИК, '')) = 'Заказ отправлен' 
                    THEN wc.Номер_сварного_шва 
                END) as заявлен_вик,

                COUNT(DISTINCT CASE 
                    WHEN TRIM(COALESCE(wc.Результаты_АКТ_ВИК, '')) = 'годен' 
                    THEN wc.Номер_сварного_шва 
                END) as годен_вик,

                COUNT(DISTINCT CASE 
                    WHEN TRIM(COALESCE(wc.Результаты_АКТ_ВИК, '')) = 'Не годен' 
                    THEN wc.Номер_сварного_шва 
                END) as не_годен_вик,

                COUNT(DISTINCT CASE 
                    WHEN wc.Результаты_АКТ_ВИК IS NULL 
                        OR TRIM(COALESCE(wc.Результаты_АКТ_ВИК, '')) = '' 
                        OR LOWER(TRIM(COALESCE(wc.Результаты_АКТ_ВИК, ''))) = 'none' 
                    THEN wc.Номер_сварного_шва 
                END) as не_подан_вик,
                ...
                SUM(CASE WHEN TRIM(COALESCE(wc.Результаты_Заключения_Стилоскопирование, '')) = 'Заказ отправлен' THEN 1 ELSE 0 END) as заявлен_стилоскопирование,

                SUM(CASE WHEN TRIM(COALESCE(wc.Результаты_Заключения_Стилоскопирование, '')) = 'годен' THEN 1 ELSE 0 END) as годен_стилоскопирование,

                SUM(CASE WHEN TRIM(COALESCE(wc.Результаты_Заключения_Стилоскопирование, '')) = 'Не годен' THEN 1 ELSE 0 END) as не_годен_стилоскопирование,

                SUM(CASE WHEN (wc.Результаты_Заключения_Стилоскопирование IS NULL OR TRIM(COALESCE(wc.Результаты_Заключения_Стилоскопирование, '')) = '' OR LOWER(TRIM(COALESCE(wc.Результаты_Заключения_Стилоскопирование, ''))) = 'none') THEN 1 ELSE 0 END) as не_подан_стилоскопирование,
            ...
            WHERE
                ...
                AND (COALESCE(wc.Номер_сварного_шва, '') NOT LIKE '%FE%' AND COALESCE(wc.Номер_сварного_шва, '') NOT LIKE '%PS%')
            ...
            logger.info("🧮 Расчет столбца ПВК с учетом процента и исключением 'Не годен' по РК...")
            ...
            logger.info("🧮 Расчет столбца МПД с исключением 'Не годен' по РК...")
    ...
    def verify_results(self):
        ...
        try:
            ...
            self.cursor.execute("""
                ...
                SELECT N_Линии, _Линия, Номер_чертежа, Всего_сваренно_сварщиком, клейма_сварщиков,
...
def run_script():
    ...
    logger.info(f"run_script() вызвана с аргументами: {sys.argv}")
    ...
    if len(sys.argv) > 1:
        ...
        logger.info(f"Найдены аргументы командной строки: {sys.argv[1:]}")

        if '--preview' in sys.argv:

            logger.info("Запускаем preview...")

            return run_script_preview()

        elif '--check-only' in sys.argv:

            logger.info("Запускаем check-only...")
    ...
    logger.info("Запускаем создание таблицы...")
